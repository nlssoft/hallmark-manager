...
class RecordQuerySet(QuerySet):
    ...
    def with_financials(self):
        ...
        allocation_total = Allocation.objects.filter(record=OuterRef("pk")).values(
            "record_id"
        ).annotate(
            total=Coalesce(
                Sum("amount"),
        ...
        advanceusage_total= AdvanceUsage.objects.filter(record=OuterRef('pk')).values(
            "record_id"
        ).annotate(
            total=Coalesce(
                Sum("amount"),
        ...
        return (
            ...
            .annotate(
                ...
                _amount= ExpressionWrapper(F("rate")* F("pcs"), output_field=DecimalField())
            ).annotate(
