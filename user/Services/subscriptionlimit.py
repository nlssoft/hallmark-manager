...
class PlanLimit:
    TRIAL_PLAN= {"max_employees": 4, "max_services": 5, "max_assignments_per_customer": 2}
...
class PlanLimitChecker:
    ...
    def assert_can_add_assignments(self, customer_ids, exclude_employee = None):
        ...
        violation = {
            ...
            if count_map(cid, 0) + 1 > limit
        }

        if violation:
            pk = Customer.object.filter(pk__in = violation).values_list("pk", flat=True)
            raise ValidationError(  f"Assignment limit ({limit}) reached for: {', '.join(pk)}")
