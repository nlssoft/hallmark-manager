...
class CustomCookieTokenRefreshView(BaseRefreshView):
    """
    At refrsh views it removes the token from response body.
    """
...
class VerifyEmailOTPView(generics.GenericAPIView):
    """After registration it verifyes the user email"""
...
class ResendVerifyEmailOTPView(APIView):
    """It sends the verify email otp agian. for registrations."""
...
class ResendEmailChangeOTPView(APIView):
    "It checks opt limit. genrate otp."
...
class VerifyEmailChangeOTPView(generics.GenericAPIView):
    """It verifys the otp for pending emails."""
